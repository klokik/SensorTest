...
#define SLAVE_ADDRESS 0xEE // the slave address (BMP085)
...
{	
	...
	RCC_APB1PeriphClockCmd(RCC_APB2Periph_I2C2, ENABLE);
	// enable clock for SCL and SDA pins
	RCC_AHB1PeriphClockCmd(RCC_AHB2Periph_GPIOB, ENABLE);
...
int main(void){
	...
	while(1)
	{
		...
		I2C_write(I2C2, 0xF4); // write one byte to the slave // Register address
		I2C_write(I2C2, 0x03); // write another byte to the slave // control register - temperature
		...
		// Read data
			// what to read
		I2C_start(I2C2, SLAVE_ADDRESS<<1, I2C_Direction_Transmitter); // start a transmission in Master transmitter mode
		I2C_write(I2C2, 0xF6); // register - lol
		I2C_stop(I2C2); // stop the transmission
			//read
		...
		received_data[1] = I2C_read_nack(I2C2); // read one byte and don't request another byte, stop transmission
